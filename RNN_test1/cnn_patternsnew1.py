...
def classifier(tau, resolution):
    return '{0:02b}'.format(int(math.ceil(tau/resolution)))


print tau, classifier(tau,resolution)
...
with tf.Session() as sess:
    ...
    while step * batch_size < training_iters:
        X, classifier = classifier(tau, resolution)
        X= tf.reshape(X,(-1,784))
        classifier = tf.reshape(classifier, (-1, 2))


        # Run optimization op (backprop)
        sess.run(optimizer, feed_dict={x: X, y: classifier(tau,resolution),
                                       keep_prob: dropout})
        if step % display_step == 0:
            # Calculate batch loss and accuracy
            loss, acc = sess.run([cost, accuracy], feed_dict={x: X,
                                                              y: classifier(tau,resolution),
                                                              keep_prob: dropout})
